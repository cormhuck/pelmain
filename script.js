...
function answerQuestion(qNum, answer) {
  if (qNum === 2) {
    // Instead of advancing straight to q3, pause and collect phone number
    document.getElementById('q2').classList.remove('active');
    openPhonePopup();
    return; // q3 will be shown after phone is submitted
  }
...
function submitPhoneNumber() {
  ...
  const isValid = /^0\d{10}$/.test(phone);

  if (!isValid) {
    error.style.display = 'block';
    input.style.border = '1px solid #e0245e';
    return;
  }
  ...
  // TODO: send `phone` to your backend/CRM here

  // Now advance to q3, same logic your app already uses
  document.getElementById('q3').classList.add('active');
  document.getElementById('qd3').classList.add('q-dot--active');
  save(KEY_QUIZ_STEP, 3);
}
...
// ═══════════════════════════════════════════════════════════════
const SEGMENTS = [
  { label: "IPhone 17",         sub: "Brand New Phone",      color: "green",  image: "assets/prize-iphone.png", isPrize: true },
  { label: "₦50,000",         sub: "Instant win",  color: "white",  image: "assets/prize-cash-50k.png", isPrize: true },
  { label: "25GB DATA",          sub: "25GB DATA",      color: "green",  image: "assets/25gb.webp",  isPrize: true },
  { label: "₦10,000",          sub: "Airtime",  color: "white",  image: "assets/prize-airtime.png",   isPrize: true },
  { label: "₦150,000",       sub: "Get money now",   color: "green",  image: "assets/prize-data.png",      isPrize: true },
  { label: "₦100,000",         sub: "cash Trasfer",  color: "white", image: "assets/prize-cash-200k.png",    isPrize: true },
  { label: "Try Again",       color: "green",  icon: "↻", isPrize: false },
  { label: "plus 1 spin",        color: "white",  icon: "➜", isPrize: false },
];
